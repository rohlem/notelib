...
struct notelib_instrument_inline_steps{
	...
	//union of struct notelib_channel* external_state and struct notelib_channel inline_state[] at &inline_steps[step_count] ; size in no way enforced
};
...
struct notelib_instrument_external_steps_inline_state{
	...
	struct notelib_processing_step_entry* external_steps;
	//alignas(NOTELIB_INSTRUMENT_STATE_DATA_ALIGNMENT) struct notelib_channel inline_state[];
};
struct notelib_instrument_external_steps_external_state{
	struct notelib_instrument_external_steps base;
	struct notelib_processing_step_entry* external_steps;
	struct notelib_channel* external_state;
};
...
static const size_t notelib_instrument_external_offsetof_state_array = NOTELIB_INTERNAL_ALIGN_TO_NEXT_ALIGNOF(sizeof(struct notelib_instrument_external_steps_inline_state), struct notelib_channel);
